...
if (!defined("PCL_TAR"))
{
  ...
  if (!isset($g_pcltar_lib_dir))
    $g_pcltar_lib_dir = LIB_DIR."extention";
...
// --------------------------------------------------------------------------------
  ...
  function PclTarHandleExtract($p_tarname, $p_file_list, &$p_list_detail, $p_mode, $p_path, $p_tar_mode, $p_remove_path)
  {
    ...
    // ----- Check the path
    if (($p_path == "") || ((substr($p_path, 1, 1) != ":") && (substr($p_path, 0, 1) != "/") && (substr($p_path, 0, 3) != "../")))
      $p_path = "./".$p_path;
...
Class PclTar {
    ...
    {
        return PclTarCreate($this->tarname, $p_filelist, "tar", $p_add_dir, $p_remove_dir);
    }

    function add($p_filelist, $p_add_dir="", $p_remove_dir="")
    {
        return PclTarAddList($this->tarname, $p_filelist, $p_add_dir, $p_remove_dir, "tar");
    }

    function extract($p_path="./", $p_remove_path="")
    {
        foreach($this->tarname as $p_tarname)
        {
            PclTarExtract($p_tarname, $p_path, $p_remove_path, "tar");
        }
    }
}
