    ...
    function Prepare($_str)
    {
        ...
        if (count($_parts) > 1)
        {
            ...
            while (count($_content))
            {
                ...
                if ($_command[1] == 'list')
                {
                    $_obj = new CTemplate_Parser_List();
                ...
                elseif ($_command[1] == 'func')
                {
                    $funcObj = new CTemplate_Parser_Func();
                    $funcObj->Parse($_content);
                    ...
                    $Funcs[$_arr['name']] = $funcObj->getFunc();
                    unset($funcObj);
                }
            }
        }
        ...
        $this->Template = new CTemplate_Parser_Block();
    ...
    function & Parse(& $_str)
    {
        ...
        while(sizeof($precontent))
        {
            ...
            if ($str[0] == '#')
            {
                ...
                if ($temp[2])
                {
                    preg_match_all('/(src|filter|link|name|func)\s*=\s*[\'"]([^\'"]*)[\'"]/i',$temp[2],$temp2);
                    $temp[3] = $temp2[1];
                    $temp[4] = $temp2[2];
                }
...
class CTemplate_Parser_Block extends CTemplate_Builder_Block
{
...
class CTemplate_Parser_List extends CTemplate_Builder_List
{
    public $Func = null;
    ...
    function Parse(& $_content)
    {
        ...
        $this->Header = new CTemplate_Parser_Block();
        ...
        while($_command[1] == 'elem' || $_command[1] == 'sep' || $_command[1] == 'alter')
        {
            ...
            {
                case 'elem':
                    $_elem = new CTemplate_Parser_Elem();
                ...
                break;
                case 'sep':
                    $_sep = new CTemplate_Parser_Elem();
                ...
                break;
                case 'alter':
                    $_alter = new CTemplate_Parser_Elem();
        ...
        $this->Footer = new CTemplate_Parser_Block();
...
class CTemplate_Parser_Elem extends CTemplate_Parser_Block
{
...
class CTemplate_Parser_Func
{
    ...
    {
        array_shift($_content);
        $_command = array_shift($_content);
        while (!is_array($_command))
        {
            $this->Func .= $_command;
            $_command = array_shift($_content);
        }
